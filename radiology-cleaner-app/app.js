...
window.addEventListener('DOMContentLoaded', function() {
    ...
    function setupEventListeners() {
        ...
        // Add listener for the new validation button
        const runValidationBtn = document.getElementById('runValidationBtn');
        if (runValidationBtn) {
            runValidationBtn.addEventListener('click', runValidationPipeline);
        }

        async function runValidationPipeline() {
            showLoadingIndicator('Generating validation data...');
            try {
                const response = await fetch(`${apiConfig.baseUrl}/generate_view_data`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ /* potentially pass parameters like job_id if needed */ })
                });

                if (!response.ok) {
                    const errorText = await response.text();
                    throw new Error(`Failed to generate view data: ${errorText}`);
                }

                const result = await response.json();
                if (result.view_data_path) {
                    // Redirect to the validation UI with the path to the generated file
                    window.location.href = `/validation_ui/index.html?view_data=${encodeURIComponent(result.view_data_path)}`;
                } else {
                    throw new Error('No view_data_path received from backend.');
                }
            } catch (error) {
                statusManager.show(`❌ Validation pipeline failed: ${error.message}`, 'error', 0);
    ...
    async function runSanityTest() {
        ...
        showLoadingIndicator('Running sanity test...');
        let statusId = null;
    ...
    async function runRandomSampleDemo() {
        ...
        showLoadingIndicator('Processing random sample demo...');
        let statusId = null;
