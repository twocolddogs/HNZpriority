...
class RadiologySemanticParser:
    ...
    def parse_exam_name(self, exam_name, modality_code, scispacy_entities=None):
        ...
        # 1. Get anatomy from NLP entities
        nlp_anatomy = scispacy_entities.get('ANATOMY', [])
        for item in nlp_anatomy:
            # Normalize NLP output by checking against our own terms
            term_key = item.lower()
            if term_key in self.anatomy_lookup:
                found_anatomy.add(self.anatomy_lookup[term_key]['standardName'])
            else:
                found_anatomy.add(item.capitalize()) # Add it anyway if not in our list
        
        # 2. Get anatomy from our detailed rule-based matching
        ...
        # If a specific vascular part is mentioned, remove the general body part to avoid redundancy
    ...
    def _build_clean_name(self, parsed):
        ...
        if parsed['anatomy']:
            ...
            parts.append("Unknown Anatomy") # Make it clear when no anatomy is found
